	...
	public class ForwardGeocodeUserInput : MonoBehaviour
	{
		...
		void HandleUserInput(string searchString)
		{
			...
			if (!string.IsNullOrEmpty(searchString))
			{
				...
				Console.WriteLine("Test HandleUserInput");
			}
			Console.WriteLine("Test HandleUser out of if");
		}
		...
		void HandleGeocoderResponse(ForwardGeocodeResponse res)
		{
			...
			if (null == res)
			{
				...
				Console.WriteLine("Test HandleGeocoderResponse if (null == res)");
			}
			else if (null != res.Features && res.Features.Count > 0)
			{
				var center = res.Features[0].Center;
				_coordinate = res.Features[0].Center;
				Console.WriteLine("Test HandleGeocoderResponse null  if (!= res.Features && res.Features.Count > 0)");
			}
			Response = res;
